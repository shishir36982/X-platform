...
$select_query = "SELECT * FROM communities where name like '%{$t}%' or createdby like '%{$t}%'";
  ...
  while($row = mysqli_fetch_assoc($select_query_result)) {?>
   <div class="card w-200">
      ...
      <div class="card-body">
       ...
       <?php echo '<a href="communities.php?id='.$row['c_id'].'" class="btn btn-primary">explore</a>  ';
  ?>
      </div>
...
$select_query = "SELECT * FROM threads where title like '%{$t}%' or createdby like '%{$t}%'";
$select_query_result = mysqli_query($con, $select_query) or die(mysqli_error($con));
$total_rows_fetched = mysqli_num_rows($select_query_result);
if($total_rows_fetched==0){
